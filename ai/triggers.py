...
class TriggerManager:
    ...
    def evaluate_command(self, context: CommandContext) -> List[Trigger]:
        ...
        if context.exit_code != 0:
            error_triggers = [t for t in self.triggers 
        ...
        if context.command_type == CommandType.DANGEROUS:
            dangerous_triggers = [t for t in self.triggers 
        ...
        pattern_triggers = [t for t in self.triggers 
    ...
    def _log_trigger(self, trigger: Trigger, context: Optional[CommandContext]):
    ...
    def get_trigger_statistics(self) -> Dict:
        ...
        recent_triggers = [e for e in self.trigger_history if e["timestamp"] > recent_time]
        
        return {
            "total_triggers": total_triggers,
            "type_distribution": type_counts,
            "recent_triggers": len(recent_triggers),
            "most_common_trigger": max(type_counts.items(), key=lambda x: x[1])[0] if type_counts else None
        }

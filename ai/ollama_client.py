...
class OllamaClient:
    ...
    async def generate(
        ...
        **options
        ...
        if self._available_models and model not in self._available_models:
            raise ValueError(f"Model '{model}' not available. Available models: {self._available_models}")
    ...
    async def _handle_single_response(self, response: httpx.Response, start_time: float) -> AIResponse:
    ...
    async def _handle_streaming_response(self, response: httpx.Response, start_time: float) -> AIResponse:

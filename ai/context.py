...
class ContextWindow:
    ...
    def __init__(self, max_commands: int = 20, max_tokens: int = 4000):
        ...
        self.session: SessionContext = SessionContext(
            current_directory="/", shell_type="bash"
        )
    ...
    def get_commands_by_type(
        self, command_type: CommandType, limit: int = 10
    ) -> List[CommandContext]:
        """Get recent commands of specific type."""
        type_commands = [
            cmd for cmd in self.commands if cmd.command_type == command_type
        ]
    ...
    def get_context_summary(self) -> str:
        ...
        if self.session.git_status:
            ...
            summary_parts.append(
                f"Git: {git['branch']} ({'clean' if not git['has_changes'] else 'modified'})"
            )
    ...
    def get_statistics(self) -> Dict:
        ...
        avg_relevance = (
            sum(cmd.relevance_score for cmd in self.commands) / total_commands
        )
...
def classify_command(command: str) -> CommandType:
    ...
    if any(
        pattern in command
        for pattern in ["rm -rf", "sudo rm", "format", "mkfs", "dd if=", "> /dev/"]
    ):
    ...
    if any(
        ...
        for net in ["curl ", "wget ", "ping ", "ssh ", "scp ", "rsync "]
    ):
        return CommandType.NETWORK

    if any(
        command.startswith(file_op)
        for file_op in [
            "cp ",
            "mv ",
            "rm ",

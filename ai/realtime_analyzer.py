...
from .context_manager import ContextManager, AIAnalysisRequest
...
class RealtimeAnalyzer:
    ...
    async def analyze_command_error(
        ...
        cache_key = self._create_cache_key("error", command, error_output, context or "")
        ...
        async with self.request_semaphore:
            try:
                ...
                await self._trigger_callbacks("error_analyzed", {
                    "command": command,
                    "response": response
                })
    ...
    async def analyze_command_output(
        ...
        async with self.request_semaphore:
            try:
                ...
                await self._trigger_callbacks("output_analyzed", {
                    "command": command,
                    "response": response
                })
    ...
    async def suggest_commands(
        ...
        async with self.request_semaphore:
            try:
                ...
                await self._trigger_callbacks("commands_suggested", {
                    "intent": intent,
                    "response": response
                })
    ...
    async def _handle_analysis_request(self, request: AIAnalysisRequest):
        ...
        try:
            ...
            if response:
                await self._trigger_callbacks("analysis_completed", {
                    "request": request,
                    "response": response
                })
            ...
            await self._trigger_callbacks("analysis_failed", {
                "request": request,
                "error": str(e)
            })

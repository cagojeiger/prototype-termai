...
class ContextFilter:
    ...
    def __init__(self):
        self.sensitive_patterns = [
            ...
            (r'postgresql://[^:]+:[^@]+@[^/]+/[^\s]+', 'postgresql://[USER]:[PASS]@[HOST]/[DB]'),
            (r'mysql://[^:]+:[^@]+@[^/]+/[^\s]+', 'mysql://[USER]:[PASS]@[HOST]/[DB]'),
            (r'mongodb://[^:]+:[^@]+@[^/]+/[^\s]+', 'mongodb://[USER]:[PASS]@[HOST]/[DB]'),
        ...
        self.sensitive_commands = {
            'env', 'printenv', 'set',  # Environment variables
            'history',  # Command history
            'cat', 'less', 'more', 'head', 'tail',  # File content (when reading sensitive files)
            'grep',  # When searching in sensitive files
            'ps', 'top',  # Process lists (may contain sensitive args)
        }
        
        self.sensitive_extensions = {
            '.key', '.pem', '.p12', '.pfx', '.crt', '.cer',  # Certificates and keys
    ...
    def _filter_history_output(self, output: str) -> str:
        ...
        for line in lines:
            ...
            if any(sensitive in cleaned_line.lower() for sensitive in [
    ...
    def _is_sensitive_file(self, filename: str) -> bool:
        ...
        sensitive_patterns = [
            ...
            'private', 'confidential', 'auth', 'login'
        ]

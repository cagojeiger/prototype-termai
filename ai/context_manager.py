...
from .context import ContextWindow, CommandContext, SessionContext, classify_command
...
class ContextManager:
            ...
            if context.exit_code == 0:
                ...
                for line in output_lines:
                    if line.startswith('On branch '):
                        ...
                        has_changes = False
                    elif any(keyword in line for keyword in ['modified:', 'new file:', 'deleted:']):
                        has_changes = True
    ...
    def _should_compress_context(self, context_list: List[CommandContext]) -> bool:
        ...
        threshold = self.context_window.max_tokens * self.config["context_compression_threshold"]
        return total_tokens > threshold
    
    async def _compress_context(self, context_list: List[CommandContext]) -> List[CommandContext]:
        """Compress context by removing less relevant commands."""
        
        sorted_context = sorted(context_list, key=lambda x: x.relevance_score, reverse=True)
